...
const ROLE_BADGE = {
  ...
  user:    { Icon: UserIcon,    cls: 'bg-background-tertiary text-foreground-secondary' },
}
...
export default function PlatformUsersOverviewPage() {
  ...
  useEffect(() => {
    let alive = true
    setLoading(true)
    platformService.listUsersOverview({
      days,
      page,
      limit,
      ...
      .catch((e) => { if (alive) setErr(e?.response?.data?.error || t('usersOverview.loadError', 'Failed to load.')) })
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-end justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-foreground-secondary mt-1">{t('usersOverview.subtitle', 'Cross-holding usage by user.')}</p>
          </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <form onSubmit={submitSearch} className="flex items-center gap-2">
            <div className="relative">
              <Search className="absolute start-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-foreground-tertiary pointer-events-none" />
              <Input
                value={searchInput}
            ...
            <Button type="submit" variant="outline" size="sm">{t('usersOverview.searchBtn', 'Search')}</Button>
          </form>
        ...
        <Card>
          <CardContent className="p-0">
            {loading ? (
              <div className="p-8 flex justify-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-accent border-t-transparent" /></div>
            ) : err ? (
              <p className="p-6 text-error">{err}</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {users.length === 0 && (
                      <tr><td colSpan={7} className="text-center py-10 text-foreground-tertiary">{t('usersOverview.empty', 'No users match.')}</td></tr>
                    )}
                    {users.map((u) => (
                      <tr key={u._id} className="border-t border-border hover:bg-background-tertiary/40">
                        <td className="px-4 py-3">
                          <div className="font-medium text-foreground">{u.display_name || u.email}</div>
                          {u.display_name && <div className="text-xs text-foreground-tertiary">{u.email}</div>}
